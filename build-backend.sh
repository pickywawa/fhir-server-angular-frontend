...
echo "🔨 Compilation de tous les services backend..."
...
if [ ! -x "$JAVA17_HOME/bin/java" ]; then
	echo "❌ Java 17 introuvable. Installe Java 17 pour compiler le backend."
	exit 1
fi
...
JAVA_HOME="$JAVA17_HOME" PATH="$JAVA17_HOME/bin:$PATH" mvn clean install -DskipTests
